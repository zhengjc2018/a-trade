...
def _fake_df():
    return pd.DataFrame([
        {"板块": "发电设备", "涨跌幅": 5.74, "总成交额": 1e10,
         "股票代码": "sz300062", "股票名称": "中能电气", "个股-涨跌幅": 20.04},
        {"板块": "电器行业", "涨跌幅": 4.96, "总成交额": 8e9,
         "股票代码": "sz300407", "股票名称": "凯发电气", "个股-涨跌幅": 14.96},
        {"板块": "下跌板块", "涨跌幅": -1.50, "总成交额": 5e9,
         "股票代码": "sz000001", "股票名称": "测试股票", "个股-涨跌幅": -1.50},
    ])
...
def test_fetch_top_gainers_sorted():
    with patch("akshare.stock_sector_spot", return_value=_fake_df()):
        ...
        assert result[0]["leader_change_pct" if False else "change_pct"] == 20.04
        assert result[0]["name"] == "中能电气"
